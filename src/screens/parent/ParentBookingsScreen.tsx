...
export default function ParentBookingsScreen() {
  ...
  const [outBookingId, setOutBookingId] = useState<string | null>(null);
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [bookings, kids] = await Promise.all([
        myBookings(),
      ...
      setList(
        bookings.sort(
          (a, b) => +new Date(b.startsAtUtc) - +new Date(a.startsAtUtc)
        )
      );
    } catch (e: any) {
  ...
  const markRead = async () => {
    if (!outBookingId) return;
    try {
      setOutBusy(true);
      await acknowledgeOutcome(outBookingId);
      setOutcome((o) =>
        o ? { ...o, parentAcknowledgedAtUtc: new Date().toISOString() } : o
      );
  ...
  return (
    <SafeAreaView style={{ flex: 1 }} edges={["top", "left", "right"]}>
      <StatusBar translucent={false} barStyle="dark-content" />
      <ScrollView
        contentContainerStyle={{ paddingHorizontal: 12, paddingBottom: 28, paddingTop: 8 }}
        ...
        {list.map((b) => {
          ...
          return (
            <View key={b.id} style={s.card}>
              ...
              {!!childLabel && (
                <Text style={s.childLine}>
                  Ребёнок:{" "}
                  <Text style={{ fontWeight: "800", color: "#2B2F2C" }}>
                    {childLabel}
                  </Text>
                </Text>
              )}
              {!!b.messageFromParent && (
                <Text style={s.note}>&quot;{b.messageFromParent}&quot;</Text>
              )}
              ...
              {b.status === BookingStatus.Completed && (
                ...
                >
                  {outBusy ? (
                    ...
                    <Text style={s.cancelTxt}>Посмотреть заключение</Text>
                  )}
                </TouchableOpacity>
              )}
            </View>
        ...
        >
          <View
            style={{
              ...
              padding: 16,
              paddingBottom: 16 + insets.bottom, // учёт домашней полоски
              maxHeight: "85%",
            }}
          >
            <View
            ...
            >
              <Text style={{ fontWeight: "900", fontSize: 18, color: "#2B2F2C" }}>
                Заключение
              </Text>
              <TouchableOpacity onPress={() => setOutModalVisible(false)}>
            ...
            {!outcome ? (
              ...
              <ScrollView contentContainerStyle={{ paddingVertical: 8 }}>
                <Text style={{ color: "#2B2F2C", fontWeight: "800", marginTop: 8 }}>
                  Итог
                </Text>
                <Text style={{ color: "#46534E", marginTop: 4 }}>
                  {outcome.summary}
                </Text>
                ...
                {!!outcome.recommendations && (
                  <>
                    <Text
                      style={{ color: "#2B2F2C", fontWeight: "800", marginTop: 12 }}
                    >
                      Рекомендации
                    </Text>
                    <Text style={{ color: "#46534E", marginTop: 4 }}>
                ...
                {outcome.parentAcknowledgedAtUtc ? (
                  <Text style={{ color: "#7A8A83", marginTop: 4, fontSize: 12 }}>
                    ...
                    {new Date(outcome.parentAcknowledgedAtUtc).toLocaleString()}
                  </Text>
                ) : null}
                ...
                {!outcome.parentAcknowledgedAtUtc && (
                  <TouchableOpacity
                    onPress={markRead}
                    style={[s.cancelBtn, { backgroundColor: PRIMARY, marginTop: 16 }]}
                  >
                    {outBusy ? (
                      <ActivityIndicator color="#fff" />
                    ) : (
                      <Text style={s.cancelTxt}>Я прочитал(а)</Text>
                    )}
                  </TouchableOpacity>
                )}
              </ScrollView>
...
function statusPill(s: BookingStatus) {
  switch (s) {
    case BookingStatus.Pending:
      return { bg: "#FFF7D1", text: "#8A6D1A", textLabel: "Ожидает" };
    case BookingStatus.Confirmed:
      return { bg: "#E8F5E9", text: "#256029", textLabel: "Подтверждено" };
    case BookingStatus.Declined:
      return { bg: "#F3F4F6", text: "#6B7280", textLabel: "Отклонено" };
    case BookingStatus.CancelledByParent:
      return { bg: "#F3F4F6", text: "#6B7280", textLabel: "Отменено (вами)" };
    case BookingStatus.CancelledBySpecialist:
      return { bg: "#F3F4F6", text: "#6B7280", textLabel: "Отменено (спец.)" };
    case BookingStatus.Completed:
      return { bg: "#E8EAF6", text: "#1F2A44", textLabel: "Завершено" };
    default:
      return { bg: "#EEE", text: "#555", textLabel: String(s) };
  }
}
